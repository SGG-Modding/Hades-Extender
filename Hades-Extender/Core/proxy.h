...
public:
	BOOL Initialize()
	{
		/* Prevent our proxy from being unloaded by `SDL` */
		/* call    cs:FreeLibrary */
		uint8_t nop[] = { 0x90,0x90,0x90,0x90,0x90,0x90 };
		DWORD protect;
		PVOID address = (PVOID)((uintptr_t)GetModuleHandleA("SDL2.dll") + 0x00DD19);
		VirtualProtect(address, 6, PAGE_EXECUTE_READWRITE, &protect);
		memcpy(address, nop, 6);
		VirtualProtect(address, 6, protect, &protect);
